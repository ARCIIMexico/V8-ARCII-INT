...
if ( ! class_exists( 'Astra_Sites_Importer' ) ) {
	...
	class Astra_Sites_Importer {
		...
		public static function get_single_demo( $demo_api_uri ) {
			...
			//// If a file with demo data exists, get the data
			$demo_id = substr( $demo_api_uri, strlen( $demo_api_uri ) - 5, 5);
			$file_name = ABSPATH .'/wp-content/plugins/astra-pro-sites/astra-sites/' . $demo_id . '.json';
			if ( file_exists( $file_name ) ) {
				$data = json_decode( file_get_contents( $file_name ), true );

				// Remove ultimate-elementor from required plugins list
				for ( $i = 0; $i < count( $data['required-plugins'] ); $i++ ) { 
					if ( $data['required-plugins'][$i]['slug'] == 'ultimate-elementor' ) {
						unset( $data['required-plugins'][$i] );
						break;
					}
				}
				$data['required-plugins'] = array_values( $data['required-plugins'] );
